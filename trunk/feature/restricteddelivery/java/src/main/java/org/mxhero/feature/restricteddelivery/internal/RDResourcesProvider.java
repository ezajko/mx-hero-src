package org.mxhero.feature.restricteddelivery.internal;
...
public class RDResourcesProvider extends StreamDRLProvider{
	...
	private static final String CHECK_OWN_DOMAIN = "$#CHECK_OWN_DOMAIN#$";
	...
	protected StringBuilder processHeader(String domain) throws IOException {
		StringBuilder sb = streamToBuilder(RDResourcesProvider.class.getClassLoader().getResourceAsStream(HEADER_FILE));
	...
	StringBuilder sb = streamToBuilder(RDResourcesProvider.class.getClassLoader().getResourceAsStream(RULE_FILE));
	String action = "";
	...
	if(ownDomain){
		sb.replace(sb.indexOf(CHECK_OWN_DOMAIN), sb.indexOf(CHECK_OWN_DOMAIN)+CHECK_OWN_DOMAIN.length(), "eval( $initialData.getSender().getDomain().getId().equals($initialData.getRecipient().getDomain().getId()) == false )");
	}else{
		sb.replace(sb.indexOf(CHECK_OWN_DOMAIN), sb.indexOf(CHECK_OWN_DOMAIN)+CHECK_OWN_DOMAIN.length(), "");
	}
