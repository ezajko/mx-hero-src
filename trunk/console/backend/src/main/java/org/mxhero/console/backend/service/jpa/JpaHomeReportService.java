...
public class JpaHomeReportService implements HomeReportService{
	...
	public MxHeroDataVO getMxHeroData(String domainId) {
		...
		if(domainId==null){
			data.setDomains(mxem.createQuery("SELECT COUNT(d) FROM Domain d", Long.class).getSingleResult());
			data.setGroups(null);
			data.setAccounts(mxem.createQuery("SELECT COUNT(ea) FROM EmailAccount ea", Long.class).getSingleResult());
			data.setEnabledRules(mxem.createQuery("SELECT COUNT(r) FROM FeatureRule r WHERE r.enabled = true", Long.class).getSingleResult());
			data.setDisableRules(mxem.createQuery("SELECT COUNT(r) FROM FeatureRule r WHERE r.enabled = false", Long.class).getSingleResult());
		}else{
			data.setDomains(null);
			data.setGroups(mxem.createQuery("SELECT COUNT(g) FROM Group g WHERE g.domain.domain = ? ", Long.class).setParameter(1, domainId).getSingleResult());
			data.setAccounts(mxem.createQuery("SELECT COUNT(ea) FROM EmailAccount ea WHERE ea.domain.domain = ? ", Long.class).setParameter(1, domainId).getSingleResult());
			data.setEnabledRules(mxem.createQuery("SELECT COUNT(r) FROM FeatureRule r WHERE r.enabled = true AND r.domain.domain = ? ", Long.class).setParameter(1, domainId).getSingleResult());
			data.setDisableRules(mxem.createQuery("SELECT COUNT(r) FROM FeatureRule r WHERE r.enabled = false AND r.domain.domain = ? ", Long.class).setParameter(1, domainId).getSingleResult());			
		}
	...
	public MessagesCompositionVO getMessagesCompositionData(long since, String domainId) {
		...
		String basicQuery = "SELECT COUNT(*) FROM mail_records r0 " 
			+" INNER JOIN mail_records r1 " 
			+" ON r1.parent_insert_date = r0.parent_insert_date " 
			+" AND r1.parent_sequence = r0.parent_sequence "
			+" AND r1.insert_date >= ? "
			+" WHERE r0.insert_date > ? "
			+" AND ((r0.phase='send' AND r0.state='drop') OR (r0.phase='receive' AND r1.phase='send')) " 
			+" AND (EXISTS( SELECT 1 FROM mail_stats s "
									+" WHERE s.insert_date = r0.insert_date " 
									+" AND s.record_sequence = r0.record_sequence " 
									+" AND ${CONDITION}) " 
					+" OR EXISTS( SELECT 1 FROM mail_stats s "
										+" WHERE s.insert_date = r1.insert_date " 
										+" AND s.record_sequence = r1.record_sequence " 
										+" AND ${CONDITION})) ";
		...
		if(domainId==null){
			messages.setSpam(((BigInteger)stem.createNativeQuery(basicQuery.replaceAll("\\$\\{CONDITION\\}", "s.stat_key='spam.detected' AND s.stat_value='true'"))
					.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).getSingleResult()).longValue());
			...
			messages.setBlocked(((BigInteger)stem.createNativeQuery(basicQuery.replaceAll("\\$\\{CONDITION\\}", "s.stat_key='email.blocked'"))
					.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).getSingleResult()).longValue());
			messages.setClean(((BigInteger)stem.createNativeQuery(cleanQuery).setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).getSingleResult()).longValue());
		}else{
			messages.setSpam(((BigInteger)stem.createNativeQuery(basicQuery.replaceAll("\\$\\{CONDITION\\}", "s.stat_key='spam.detected' AND s.stat_value='true'").concat(" AND (r0.recipient_domain_id = ? OR r0.sender_domain_id = ?) "))
					.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).setParameter(3, domainId).setParameter(4, domainId).getSingleResult()).longValue());
			...
			messages.setBlocked(((BigInteger)stem.createNativeQuery(basicQuery.replaceAll("\\$\\{CONDITION\\}", "s.stat_key='email.blocked'").concat(" AND (r0.recipient_domain_id = ? OR r0.sender_domain_id = ?) "))
					.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).setParameter(3, domainId).setParameter(4, domainId).getSingleResult()).longValue());
			messages.setClean(((BigInteger)stem.createNativeQuery(cleanQuery.concat(" AND (r0.recipient_domain_id = ? OR r0.sender_domain_id = ?) ")).setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).setParameter(3, domainId).setParameter(4, domainId).getSingleResult()).longValue());			
		}
	...
	public ActivityDataVO getActivity(long since, String domainId) {
		...
		String threatsQuery = "SELECT COUNT(*), date_format(r0.insert_date,'%Y-%m-%d %H:%i') FROM mail_records r0 " 
				...
				+" AND (EXISTS( SELECT 1 FROM mail_stats s "
						...
						+" OR EXISTS( SELECT 1 FROM mail_stats s "
											...
											+" AND s.record_sequence = r1.record_sequence " 
											+" AND ${CONDITION})) ";
		String trafficQuery =" SELECT COUNT(*), date_format(r0.insert_date,'%Y-%m-%d %H:%i') " +
				" FROM mail_records r0 INNER JOIN mail_records r1 " +
				" ON r1.parent_insert_date = r0.parent_insert_date  " +
				" AND r1.parent_sequence = r0.parent_sequence  " +
				" AND r1.insert_date >= ? " +
				" WHERE r0.insert_date >= ? " +
		...
		if(domainId==null){
			...
			.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).getResultList());
			...
			data.setIncomming(stem.createNativeQuery(trafficQuery.concat(incomming).concat(" AND r0.recipient_domain_id = ? ").concat(group))
					.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).setParameter(3, domainId).getResultList());
					...
					data.setSpam(stem.createNativeQuery(threatsQuery.concat(" AND (r0.recipient_domain_id = ? OR r0.sender_domain_id = ?) ").concat(group).replaceAll("\\$\\{CONDITION\\}", "s.stat_key='spam.detected' AND s.stat_value='true'"))
							.setParameter(1, new Timestamp(since)).setParameter(2, new Timestamp(since)).setParameter(3, domainId).setParameter(4, domainId).getResultList());
