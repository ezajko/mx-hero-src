package org.mxhero.engine.plugin.adsync.repository.jdbc;
...
public class JDBCDomainAdLdapRepository implements DomainAdLdapRepository {
	...
	public DomainAdLdap findDomainAdLdap(String domainId){
		String sql = " SELECT da.domain, da.address, da.base, da.directory_type, " +
				...
				" FROM domain_adldap da INNER JOIN app_users au ON da.domain = au.domain " +
				" WHERE da.domain = :domainId ";
		DomainAdLdap domainAdLdap = template.queryForObject(sql, new MapSqlParameterSource("domainId", domainId), new RowMapper<DomainAdLdap>() {
