...
import org.mxhero.engine.core.internal.pool.task.SenderRuleTask;
...
public final class SendPool extends QueueTaskPool implements Observer{
	...
	public SendPool(MimeMailQueueService queueService) {
		super(Mail.Phase.send,queueService);
			...
			if(loader.getBuilder().getBase()==null){
				...
				return new Runnable() {
					...
					public void run() {
						...
						try {
							queueService.delayAndPut(SendPool.this.getPhase(), mail, delayTime);
